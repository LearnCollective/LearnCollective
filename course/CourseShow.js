...
const CourseShow=({navigation,route})=>{
    const presshandler = () => {
        navigation.navigate('Home');
      }
    const {course,SharedElementPrefix}=route.params;
   ...
   function backhand (){
    ...
    function renderHeder(){
        ...
        <Animated.View style={[{
            ...
            >
            <SharedElement
           id={`${SharedElementPrefix}-course-Bg-${course.id}`}
           ...
           <Animated.View style={{position:'absolute' ,bottom:70,left:30}}>
            <SharedElement id={`${SharedElementPrefix}-course-Title-${course.id}`}
             style={[StyleSheet.absoluteFillObject]}>
...
CourseShow.SharedElement=(route,otherRoute,showing)=>{
    ...
    return[
     ...
     {
        id:`${SharedElementPrefix}-course-Title-${course?.id}`
     }
]
    }
